...
class Ant(Insect):
    ...
    def buff(self):
        ...
        # BEGIN Problem 12
        if self.already_buffed == False: 
            self.damage = self.damage * 2
            self.already_buffed = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        start = self.place
        location = 0
        while start.is_hive != True:
            if start.bees == []:
                start = start.entrance
                location +=1
            else:
                if self.min_range <= location <= self.max_range:
                    return random_bee(start.bees)
                else:
                    start = start.entrance
                    location += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if health <= 0:
            for b in bees_inplace:
                Insect.reduce_health(b, self.damage + amount) #Reduces the health of the bee by 
            self.remove_ant
        else:
            for b in bees_inplace:
...
class HungryAnt(Ant): 
    ...
    def action(self, gamestate):
        if self.chew_countdown > 0: 
            ...
            bee = random_bee(list(self.place.bees))
            if bee: 
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if QueenAnt.possible_to_construct: 
            QueenAnt.possible_to_construct = False 
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.is_slowed and self.is_scared:
        ...
        elif self.is_slowed and self.slowed_countdown > 0:
            self.slowed_countdown -= 1
            if self.slowed_countdown <= 0:
                self.is_slowed = False
            if gamestate.time % 2 == 0:
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2):
        ...
        dimensions -- is_a pair containing the dimensions of the game layout
        """
        self.time = 0
        self.food = food

...
def choose(paragraphs, select, k):
    ...
    for i in range(len(paragraphs)):
        if select(paragraphs[i]):
            true_paragraphs.append(paragraphs[i])
    
    if len(true_paragraphs) - 1 < k:
        return ""
...
def wpm(typed, elapsed):
    ...
    if typed == 0:
        return 0 
    minutes = elapsed / 60 
    return (len(list(typed)) / 5) / minutes
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    match_index = 0
    match_diff = diff_function(typed_word, valid_words[0], limit)
    for i in range(len(valid_words)):
        if valid_words[i] == typed_word:
            return typed_word
        if diff_function(typed_word, valid_words[i], limit) < match_diff: 
            match_index = i
            match_diff = diff_function(typed_word, valid_words[i], limit)
    if match_diff > limit:
        return typed_word
    else:
        return valid_words[match_index]
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            break 
    d = { "id": user_id, "progress": (num_correct / len(prompt))}
    upload(d)
    return d['progress']
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        lst = [(player[i + 1]-player[i]) for i in range(len(player)-1)]
        times_per.append(lst)   
    return match(words, times_per)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for x in word_indices:
        best_times_by_word.append(times[0][x])
        best_player_by_word.append(0)
    
    for i in word_indices:
        for x in player_indices:
            if times[x][i] < best_times_by_word[i]:
                best_times_by_word[i] = times[x][i]
                best_player_by_word[i] = x

    fastest = []
    for player in player_indices:
        fastest.append([word_at(match, i) for i in range(len(best_player_by_word)) if best_player_by_word[i] == player])
    return fastest 
    # END PROBLEM 10

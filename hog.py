...
def picky_piggy(score):
    ...
    num = score % 6
    if num != 0:
        return int(1/7 % 10**(-(num-1)) // 10**(-num))
    else:
        return 7
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            score0 += take_turn(strategy0(score0, score1), score1, dice, GOAL_SCORE) 
            score0 += hog_pile(score0,score1)
        elif who == 1: 
            score1 += take_turn(strategy1(score1, score0), score0, dice, GOAL_SCORE) 
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            curr_score = score1
        if  running_high < (curr_score - last_score):
            print(str(curr_score - last_score) + " point(s)! That's a record gain for Player "+ str(who) + "!")
            return announce_highest(who, curr_score, curr_score - last_score)
        else:
            return announce_highest(who, curr_score, running_high)
    return say
    # END PROBLEM 7
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if (picky_piggy_strategy(score, opponent_score, cutoff, num_rolls) == 0):
        return 0
    elif hog_pile(score + picky_piggy(opponent_score), opponent_score) != 0:
        return 0
    else:
        return num_rolls
    #return 6  # Remove this line once implemented.
